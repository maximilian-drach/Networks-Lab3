...
def QoE_Score_Static_Chunk(client_message, video_quality_Rk_int, video_quality_Rk_chunck, video_quality_Rk_1, C_estimate, C_longterm, avg_future_bitrate, buffer_occupancy, buffer_growth_per_chunk_limit):
    # print(client_message.rebuffering_coefficient*sum([((video_quality_Rk_chunck[i] / C_estimate) - (buffer_occupancy)) for i in range(len(video_quality_Rk_chunck))]))
    #change max to -12 
    # QoE = ((client_message.quality_coefficient*sum(video_quality_Rk_chunck)/len(video_quality_Rk_int))
    QoE = ((client_message.quality_coefficient*sum(video_quality_Rk_int)/len(video_quality_Rk_int))
    - (client_message.variation_coefficient*sum([abs(video_quality_Rk_1[i] - video_quality_Rk_int[i]) for i in range(len(video_quality_Rk_int))]) / len(video_quality_Rk_int))
    - client_message.rebuffering_coefficient*sum([(max((video_quality_Rk_chunck[i] / C_estimate) - (buffer_occupancy+i), -buffer_growth_per_chunk_limit) - math.floor((avg_future_bitrate / C_longterm)-(buffer_occupancy))) for i in range(len(video_quality_Rk_chunck))])/len(video_quality_Rk_int)
    )
    return QoE

def Robust_MPC(client_message, C_estimate, C_longterm, avg_future_bitrate, average_buffer_growth):
    ...
    for i in range(len(video_quality_Rk_MDP_list)):
        ...
        temp_QoE = QoE_Score_Static_Chunk(client_message, index_list[i], video_quality_Rk_MDP_list[i], index_list[i], C_estimate, C_longterm, avg_future_bitrate, buffer_occupancy, average_buffer_growth)
        # print(temp_QoE)
