...
def QoE_Score_Static_Chunk(client_message, video_quality_Rk_int, video_quality_Rk_chunck, C_estimate, buffer_occupancy):
    ...
    - client_message.rebuffering_coefficient*sum([(max((video_quality_Rk_chunck[i] / C_estimate) - (buffer_occupancy+(i*.1)), -1))for i in range(len(video_quality_Rk_chunck))])
    - client_message.audio_coefficient*(sum([max((video_quality_Rk_chunck[0] / C_estimate) - (buffer_occupancy + (i*.1)), -1) for i in range(len(video_quality_Rk_chunck))]))
    )
    return QoE

   
             
def MPC_voice_bitrate_algorithm(client_message, C_esti, prev_bitrates_vals):
    index_list = index_list_creator(len(prev_bitrates_vals[0]),  len(prev_bitrates_vals))
    video_quality_Rk_MDP_list = MDP_bitrate_chuck_list(index_list, prev_bitrates_vals)
...
def student_entrypoint(client_message: ClientMessage):
    ...
    if client_message.total_seconds_elapsed > 0:
        ...
        if len(client_message.upcoming_quality_bitrates) > 0:
            Best_QoE, Best_QoE_List = MPC_voice_bitrate_algorithm(client_message, C_Estimate, prev_bitrate_list)
            # print(f"Best QoE {Best_QoE}, QoE List {Best_QoE_List}")
            # print(Best_QoE_List, client_message.quality_bitrates)
            bitrate_choice = client_message.quality_bitrates.index(Best_QoE_List[-1])
...
##################################################
#show box plot, of time w/o audio box plot
#show evidince between 4 (robust mpc with cheating), 8.1 (MPC with out audio), 8.2(MPC with audio), showing with smaller buffers, previous perperfomanece matter most, cuz buffer will fill up quickly
#algorithm changes
#also talk about how good robust mpc at different enviorments vs bba
#MPC performace increase wqith audio variable 
##################################################