...
def read_test(config_path: str, print_output: bool):
	...
	try:
		...
		chunk_qualities = [[.128]+[(c * (2**i) * base_chunk_cost) + .128 for i in range(quality_levels)] for c in chunks]
		...
		logger = Scorecard.Scorecard(quality_coefficient, rebuffering_coefficient, variation_coefficient, audio_coefficient, chunk_length)
...
def main(config_file: str, student_algo, verbose: bool, print_output=True) -> Tuple[float, float, float, float]:
	...
	for chunknum in range(len(chunk_qualities)):
		...
		message.audio_coefficient = logger.audio_coeff
